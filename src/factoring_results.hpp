...
namespace cutrialdive {
    ...
    public:
        ...
        /// Number of factor sets i.e. this instance holds the factors of S(k) for all k in [n0, n0 + size()[.
...
#if defined CUTRIALDIVE_TRACES
        std::cout << "Adding factors (offset=" << offset << ", length=" << length << "):";
        std::for_each(std::begin(newFactors), std::end(newFactors), [](auto const & factor) {
                std::cout << " " << factor;
            });
        std::cout << std::endl;
#endif
...
#if defined CUTRIALDIVE_TRACES
        std::cout << "n0 = " << indexOfFirstNumber << std::endl;
        std::cout << "number of factorizations = " << results.size() << std::endl;
#endif
